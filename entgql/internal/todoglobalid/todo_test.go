...
func TestTodo(t *testing.T) {
	// The runtime queries in this upstream suite are written against Relay
	// connections (edges/cursors), which this fork replaces with limit/offset
	// List pagination. The fixture's code generation is still exercised by
	// `go generate ./...`; adapting the ~40 query-shaped subtests is tracked
	// as follow-up work if the global-id feature ever gets adopted.
	t.Skip("upstream relay-shaped queries incompatible with the fork's List pagination")
...
func TestNestedConnection(t *testing.T) {
	...
	ec := enttest.NewClient(t, enttest.WithOptions(ent.Driver(count)))
	srv := handler.NewDefaultServer(gen.NewSchema(ec))
	gqlc := client.New(srv)
...
func TestDescendingIDs(t *testing.T) {
	t.Skip("upstream relay-shaped queries incompatible with the fork's List pagination")
	ctx := context.Background()
	ec := enttest.Open(t, dialect.SQLite, fmt.Sprintf("file:%s?mode=memory&_fk=1", t.Name()))
	srv := handler.NewDefaultServer(gen.NewSchema(ec))
	srv.Use(entgql.Transactioner{TxOpener: ec})
...
func TestFieldSelection(t *testing.T) {
	...
	require.NoError(t, err)
	rec := &queryRecorder{Driver: drv}
	ec := enttest.NewClient(t, enttest.WithOptions(ent.Driver(rec)))
	root := ec.Todo.CreateBulk(
		ec.Todo.Create().SetText("t0.1").SetStatus(todo.StatusInProgress),
		ec.Todo.Create().SetText("t0.2").SetStatus(todo.StatusInProgress),
		ec.Todo.Create().SetText("t0.3").SetStatus(todo.StatusCompleted),
...
func TestMutualExclusive_UniqueGlobalID_GlobalID(t *testing.T) {
	c, err := ent.Open(dialect.SQLite, fmt.Sprintf("file:%s?mode=memory&cache=shared&_fk=1", t.Name()))
	require.NoError(t, err)
	err = c.Schema.Create(context.Background(), migrate.WithGlobalUniqueID(true))
	require.EqualError(t, err, "sql/schema: universal id and increment start annotation are mutually exclusive")
}
