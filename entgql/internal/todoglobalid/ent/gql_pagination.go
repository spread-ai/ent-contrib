...
// BillProductList is the connection containing edges to BillProduct.
type BillProductList struct {
	Edges      []*BillProductEdge `json:"edges"`
	Items      []*BillProduct     `json:"items"`
	PageInfo   PageInfo           `json:"pageInfo"`
	TotalCount int                `json:"totalCount"`
}

func (c *BillProductList) build(nodes []*BillProduct, pager *billproductPager, after *Cursor, first *int, before *Cursor, last *int, withPageInfo bool) {
	c.Items = nodes
...
// Paginate executes the query and returns a limit/offset paginated connection to BillProduct.
func (bp *BillProductQuery) PaginateLimitOffset(
	ctx context.Context, limit *int, offset *int, opts ...BillProductPaginateOption,
) (*BillProductList, error) {
	if err := validateLimitOffset(limit, offset); err != nil {
		return nil, err
	}
	pager, err := newBillProductPager(opts, false)
	if err != nil {
		return nil, err
	}
	...
	conn := &BillProductList{Edges: []*BillProductEdge{}}
...
// FriendshipList is the connection containing edges to Friendship.
type FriendshipList struct {
	Edges      []*FriendshipEdge `json:"edges"`
	Items      []*Friendship     `json:"items"`
	PageInfo   PageInfo          `json:"pageInfo"`
	TotalCount int               `json:"totalCount"`
}

func (c *FriendshipList) build(nodes []*Friendship, pager *friendshipPager, after *Cursor, first *int, before *Cursor, last *int, withPageInfo bool) {
	c.Items = nodes
...
// Paginate executes the query and returns a limit/offset paginated connection to Friendship.
func (f *FriendshipQuery) PaginateLimitOffset(
	ctx context.Context, limit *int, offset *int, opts ...FriendshipPaginateOption,
	...
	conn := &FriendshipList{Edges: []*FriendshipEdge{}}
	ignoredEdges := !hasCollectedField(ctx, edgesField) && !hasCollectedField(ctx, itemsField)
	if hasCollectedField(ctx, totalCountField) {
		hasPagination := limit != nil || offset != nil
		if hasPagination || ignoredEdges {
...
// GroupList is the connection containing edges to Group.
...
func (c *GroupList) build(nodes []*Group, pager *groupPager, after *Cursor, first *int, before *Cursor, last *int, withPageInfo bool) {
	c.Items = nodes
...
// Paginate executes the query and returns a limit/offset paginated connection to Group.
func (gr *GroupQuery) PaginateLimitOffset(
	ctx context.Context, limit *int, offset *int, opts ...GroupPaginateOption,
) (*GroupList, error) {
	if err := validateLimitOffset(limit, offset); err != nil {
		return nil, err
	}
	...
	conn := &GroupList{Edges: []*GroupEdge{}}
	ignoredEdges := !hasCollectedField(ctx, edgesField) && !hasCollectedField(ctx, itemsField)
	if hasCollectedField(ctx, totalCountField) {
		hasPagination := limit != nil || offset != nil
		if hasPagination || ignoredEdges {
...
// OneToManyList is the connection containing edges to OneToMany.
...
func (c *OneToManyList) build(nodes []*OneToMany, pager *onetomanyPager, after *Cursor, first *int, before *Cursor, last *int, withPageInfo bool) {
	c.Items = nodes
...
// Paginate executes the query and returns a limit/offset paginated connection to OneToMany.
func (otm *OneToManyQuery) PaginateLimitOffset(
	ctx context.Context, limit *int, offset *int, opts ...OneToManyPaginateOption,
) (*OneToManyList, error) {
	if err := validateLimitOffset(limit, offset); err != nil {
		return nil, err
	}
	pager, err := newOneToManyPager(opts, false)
	if err != nil {
		return nil, err
	}
	...
	conn := &OneToManyList{Edges: []*OneToManyEdge{}}
	ignoredEdges := !hasCollectedField(ctx, edgesField) && !hasCollectedField(ctx, itemsField)
	if hasCollectedField(ctx, totalCountField) {
		hasPagination := limit != nil || offset != nil
		if hasPagination || ignoredEdges {
	...
	if field := collectedField(ctx, edgesField, nodeField); field != nil {
		if err := otm.collectField(ctx, (limit != nil && *limit == 1), graphql.GetOperationContext(ctx), *field, []string{edgesField, nodeField}); err != nil {
			return nil, err
		}
	}
	if field := collectedField(ctx, itemsField); field != nil {
		if err := otm.collectField(ctx, (limit != nil && *limit == 1), graphql.GetOperationContext(ctx), *field, []string{itemsField}); err != nil {
			return nil, err
		}
	}
...
func (otm *OneToManyQuery) Paginate(
	...
	if field := collectedField(ctx, itemsField); field != nil {
		if err := otm.collectField(ctx, limit == 1, graphql.GetOperationContext(ctx), *field, []string{itemsField}); err != nil {
			return nil, err
		}
	}
	otm = pager.applyOrder(otm)
...
// ProjectList is the connection containing edges to Project.
...
func (c *ProjectList) build(nodes []*Project, pager *projectPager, after *Cursor, first *int, before *Cursor, last *int, withPageInfo bool) {
	c.Items = nodes
...
// Paginate executes the query and returns a limit/offset paginated connection to Project.
func (pr *ProjectQuery) PaginateLimitOffset(
	ctx context.Context, limit *int, offset *int, opts ...ProjectPaginateOption,
) (*ProjectList, error) {
	if err := validateLimitOffset(limit, offset); err != nil {
		return nil, err
	}
	pager, err := newProjectPager(opts, false)
	if err != nil {
		return nil, err
	}
	...
	conn := &ProjectList{Edges: []*ProjectEdge{}}
	ignoredEdges := !hasCollectedField(ctx, edgesField) && !hasCollectedField(ctx, itemsField)
	if hasCollectedField(ctx, totalCountField) {
		hasPagination := limit != nil || offset != nil
		if hasPagination || ignoredEdges {
	...
	if field := collectedField(ctx, edgesField, nodeField); field != nil {
		if err := pr.collectField(ctx, (limit != nil && *limit == 1), graphql.GetOperationContext(ctx), *field, []string{edgesField, nodeField}); err != nil {
			return nil, err
		}
	}
	if field := collectedField(ctx, itemsField); field != nil {
		if err := pr.collectField(ctx, (limit != nil && *limit == 1), graphql.GetOperationContext(ctx), *field, []string{itemsField}); err != nil {
			return nil, err
		}
	}
...
func (pr *ProjectQuery) Paginate(
	...
	if field := collectedField(ctx, itemsField); field != nil {
		if err := pr.collectField(ctx, limit == 1, graphql.GetOperationContext(ctx), *field, []string{itemsField}); err != nil {
			return nil, err
		}
	}
	pr = pager.applyOrder(pr)
...
// UserList is the connection containing edges to User.
...
func (c *UserList) build(nodes []*User, pager *userPager, after *Cursor, first *int, before *Cursor, last *int, withPageInfo bool) {
	c.Items = nodes
...
// Paginate executes the query and returns a limit/offset paginated connection to User.
func (u *UserQuery) PaginateLimitOffset(
	ctx context.Context, limit *int, offset *int, opts ...UserPaginateOption,
) (*UserList, error) {
	if err := validateLimitOffset(limit, offset); err != nil {
		return nil, err
	}
...
// OrganizationList is the connection containing edges to Organization.
...
func (c *OrganizationList) build(nodes []*Organization, pager *organizationPager, after *Cursor, first *int, before *Cursor, last *int, withPageInfo bool) {
	c.Items = nodes
...
// Paginate executes the query and returns a limit/offset paginated connection to Organization.
func (w *WorkspaceQuery) PaginateLimitOffset(
	ctx context.Context, limit *int, offset *int, opts ...OrganizationPaginateOption,
	...
	conn := &OrganizationList{Edges: []*OrganizationEdge{}}
	ignoredEdges := !hasCollectedField(ctx, edgesField) && !hasCollectedField(ctx, itemsField)
	if hasCollectedField(ctx, totalCountField) {
		hasPagination := limit != nil || offset != nil
		if hasPagination || ignoredEdges {
	...
	if field := collectedField(ctx, edgesField, nodeField); field != nil {
		if err := w.collectField(ctx, (limit != nil && *limit == 1), graphql.GetOperationContext(ctx), *field, []string{edgesField, nodeField}); err != nil {
			return nil, err
		}
	}
	if field := collectedField(ctx, itemsField); field != nil {
		if err := w.collectField(ctx, (limit != nil && *limit == 1), graphql.GetOperationContext(ctx), *field, []string{itemsField}); err != nil {
			return nil, err
		}
	}
...
func (w *WorkspaceQuery) Paginate(
	...
	if field := collectedField(ctx, itemsField); field != nil {
		if err := w.collectField(ctx, limit == 1, graphql.GetOperationContext(ctx), *field, []string{itemsField}); err != nil {
			return nil, err
		}
	}
	w = pager.applyOrder(w)
