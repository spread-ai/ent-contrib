...
func (e *schemaGenerator) buildWhereInput(t *gen.Type, nodeGQLType, gqlType string, genEdges bool) (*ast.Definition, error) {
	def := &ast.Definition{
		...
		Description: fmt.Sprintf("%s is used for filtering %s objects.", gqlType, t.Name),
...
func (e *schemaGenerator) buildMutationInputs(t *gen.Type, ant *Annotation, gqlType string) ([]*ast.Definition, error) {
	...
	for _, i := range ant.MutationInputs {
		...
		if def.Description == "" {
			if i.IsCreate {
				def.Description = fmt.Sprintf("%s is used for create %s object.", name, gqlType)
			} else {
				def.Description = fmt.Sprintf("%s is used for update %s object.", name, gqlType)
			}
		}
